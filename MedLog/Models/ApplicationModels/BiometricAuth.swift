    ...
    class func authenticate(completion: @escaping (Error?, Bool) -> Void) {
        guard BiometricAuth.isSupported() else {
            DispatchQueue.main.async {
                completion(NSError(domain: "BIOMETRIC_AUTH", code: 201001, userInfo: [ NSLocalizedDescriptionKey : "Biometric ID authentication failed".localized() ]), false)
            }
            
            return
        }
        
        BiometricAuth.context.evaluatePolicy(.deviceOwnerAuthentication, localizedReason: "Fingerprint authentication".localized()) { success, evaluateError in
            DispatchQueue.main.async {
